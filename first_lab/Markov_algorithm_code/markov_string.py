...
class MarkovString:
    ...
    def replacing_one_string_with_another(self, search_string, replace_string, stop_symbol):
        ...
        start_index = self.markov_string.find(search_string)
        if start_index == -1:
            return False
        ...
        new_string = self.markov_string[0:start_index]+replace_string+self.markov_string[start_index+\
            len_search_string:]
        self.markov_string = new_string
        return True

    def set_string_value(self, string_input):
        """Эта функция присваивает новое значение строке"""
        self.markov_string = string_input
    def __str__(self):
        """Эта функция выводит получившуюся строку"""
        return self.markov_string

# stri = MarkovString("hello world")
# stri.replacing_one_string_with_another("hel", "")
# print(stri.markov_string)
