...
public:
    ...
    std::shared_ptr<Reaction> reaction = std::make_shared<Reaction>("like", sh_user1);
    std::shared_ptr<Reaction> reaction2 = std::make_shared<Reaction>("love", sh_user2);
};
...
TEST_F(TestChatsMessageAndGroups, TestAddReaction) {
    message.AddReaction(reaction);
    message.AddReaction(reaction2);
    ASSERT_EQ(message.GetReaction(sh_user1), reaction);
}

TEST_F(TestChatsMessageAndGroups, TestDeleteReaction) {
    message.AddReaction(reaction);
    message.AddReaction(reaction2);
    ASSERT_THROW(
        message.DeleteReaction(sh_user3),
        ExceptionAccess
    );
    message.DeleteReaction(sh_user2);
    ASSERT_THROW(
        message.GetReaction(sh_user2),
        ExceptionAccess
    );
}
