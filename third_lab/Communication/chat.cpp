...
Message::Message(const std::string &message, const User &user): author_(user) {
	CreateMessage(message, user);
}

bool Message::CreateMessage(const std::string &message, const User &user) {
	try {
		if (message.empty()) {
...
bool BaseChat::WriteMessage(const std::string &message, const User &sender_user) {
...
Chat::Chat(const User &active_user, const User &second_user)
...
Group::Group(const User &main_user): main_user_(main_user) {
...
int Group::CheckUserExist(const std::string &username) {
