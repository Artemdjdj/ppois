...
public:
    ...
    void CreateNewStory(const std::string &name, const std::string &info, const StorySettings &settings, const std::shared_ptr<User> &user);
    std::vector<std::shared_ptr<Story>> GetAllStories(std::shared_ptr<User> user);
    ...
    void CreateNewPost(const std::string &name, const std::string &info, const std::shared_ptr<User> &user);
    std::vector<std::shared_ptr<Post>> GetAllPosts(const std::shared_ptr<User> &user);
    std::shared_ptr<Post> GetPost(const std::shared_ptr<User> &user, const std::string& id);
    void DeletePost(const std::shared_ptr<User> &user, const std::string& id);


private:
    GeneratorId stories_generator_id;
    GeneratorId posts_generator_id;
