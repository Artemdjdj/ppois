...
void PollWithoutRefactoringChoose::AddAnswer(int number_of_answer, const User *user) {
	if (number_of_answer > answers_.size() or number_of_answer < 1) {
		DefaultProjectSettings::LogFile("Incorrect answer, such answer is not in list of answers", main_log_file);
		return;
	} else if (GetAnswerOfUser(user) != -1) {
		DefaultProjectSettings::LogFile("You already voted!", main_log_file);
		return;
	}
...
void PollWithRefactoringChoose::RefactorYourChoose(const User *user, int new_answer) {
	...
	const std::string old_answer_str = this->answers_[number_of_answer - 1];
