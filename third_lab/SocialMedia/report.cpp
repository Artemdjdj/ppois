...
void Report::SetCause(const std::string &cause, const std::string &sender) {
    PropertySetter::SetValueWithAuthor(this->cause_, cause, this->sender_, sender, "Incorrect sender", "Cause can't be empty");
}

void Report::SetProof(const std::string &proof, const std::string &sender) {
    PropertySetter::SetValueWithAuthor(this->proof_, proof, this->sender_, sender, "Incorrect sender", "Proof can't be empty");
}
