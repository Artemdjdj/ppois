...
class ConstBidirectionalAdjacentVerticesIterator(BidirectionalAdjacentVerticesIterator):
    __slots__ = ()

    def __init__(self, matrix: List[List[int]], order_edges: List[int], edges: Dict[int, Edge], order_vertex: List[int],
                 vertex: int, reverse: bool = False):
        ...
        object.__setattr__(self, '_vertex_id', vertex)
        object.__setattr__(self, '_vertex', order_vertex.index(vertex))
    ...
    def __iter__(self) -> Self:
        return ConstBidirectionalAdjacentVerticesIterator(
            self._matrix,
            self._order_edges,
            self._edges,
            self._order_vertex,
