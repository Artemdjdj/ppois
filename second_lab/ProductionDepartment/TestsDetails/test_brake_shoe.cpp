...
TEST_F(TestingBrakeShoe, TestSetColor) {
    const auto color = Color("green");
    brake_shoe.SetColor(color);
    ASSERT_EQ(brake_shoe.GetColor(), "green");
}

TEST_F(TestingBrakeShoe, TestSetIncorrectColor) {
    ASSERT_THROW(
        const auto color = Color("greensdsfsd");
        brake_shoe.SetColor(color),
